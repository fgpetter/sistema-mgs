    ...
    <div class="card-body">
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane {{ session('activeTab') == 'documentos' ? 'active show' : '' }}" id="documentos"
                    ...
                    <div class="row gy-3">
                        ...
                        <div class="col-sm-4">
                            <x-forms.input-field :value="old('titulo_eleitor_cessao') ?? ($funcionario->titulo_eleitor_cessao ?? null)" type="number" name="titulo_eleitor_cessao"
                                label="Sessão Eleitoral" />
                            @error('titulo_eleitor_cessao')
                                <div class="text-warning">{{ $message }}</div>
                            @enderror
                        </div>
        ...
        @if ($funcionario->id)
            ...
            @foreach ($funcionario->dadosBancarios as $banco)
                ...
                <form id="banco-delete-{{ $banco->uid }}" method="POST"
                    action="{{ route('conta-delete', $banco->uid) }}">@csrf</form>
            @endforeach
            ...
            @foreach ($funcionario->dependentes as $dependente)
                ...
                <form id="dependente-delete-{{ $dependente->uid }}" method="POST"
                    action="{{ route('dependente-delete', $dependente->uid) }}">@csrf</form>
            @endforeach
